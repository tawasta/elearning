    ...
    def _compute_next_retry_local(self):
        for rec in self:
            if rec.next_retry_possible_at:
                dt_aware = fields.Datetime.context_timestamp(
                    self.env.user, rec.next_retry_possible_at
                )
                rec.next_retry_local = (
                    dt_aware.replace(tzinfo=None) if dt_aware else False
                )
            else:
