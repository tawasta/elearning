...
class SlidesChooseCategoryWizard(models.TransientModel):
    ...
    slide_ids = fields.Many2many(
        "slide.slide", string="Contents", default=lambda self: self.default_slides()
    )
    channel_ids = fields.Many2many(
        "slide.channel", default=lambda self: self.default_channels()
    )
    category_sort_id = fields.Many2one(
        "slide.slide",
        string="Choose a section",
    ...
    def category_sort_id_domain(self):
        return "[('is_category', '=', True), ('channel_id', 'in', {})]".format(
            self.new().channel_ids.ids
        )
